...
This specific file contains a slightly modified version of the MATLAB files found
in muse.util, with an extra `r` parameter added to determine the effect of tweaking
a certain scale factor on runtime and accuracy.
"""
...
def rsrp_from_dfted_clip_and_delays_fast(
    ...
    xcorr_raw, tau_line = xcorr_raw_from_dfted_clip(V, dt, M, r=r, verbosity=verbosity)
...
def rsrp_grid_from_clip_and_xy_grids(
    ...
    rsrp, a, xcorr_per_pair = rsrp_from_dfted_clip_and_delays_fast(V_filt, dt, tau, verbosity, r=r)
...
def xcorr_raw_from_dfted_clip(V: np.ndarray, dt: float, M: np.ndarray, r=8, verbosity=0):
    """
    ...
    Returns:
        ...
        xcorr_raw: Array of shape (N*r, n_pairs) storing the cross-
...
def r_est_jackknife(
    ...
    _, _, _, _, _, _, _, _, xcorr_per_pair_grid = r_est_from_clip_simplified(
        ...
        in_cage, mic_positions.T, verbosity=0, r=r
        )
